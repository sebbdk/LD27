package dk.sebb.tiled.layers
{
	...
	public dynamic class Layer extends EventDispatcher
	{
		...
		public function draw():void {
			if(this.perspective && this.perspective === "true") {
				drawObjects();
			} else {
				drawStatic();
			}
		...
		public function drawStatic():void {
		...
		public function drawObjects():void {
			for (var spriteForX:int = 0; spriteForX < tmxLoader.mapWidth; spriteForX++) {
				for (var spriteForY:int = 0; spriteForY < tmxLoader.mapHeight; spriteForY++) {
					...
					if(tileGid === 0) {
						//This means the first tile in the first tileset cannot be a tileobject
						continue;/////FIX ME
					}
					...
					var tile:TMXObject = new TMXObject();
					tile.x = (spriteForX) * tmxLoader.tileWidth;
					tile.y = (spriteForY) * tmxLoader.tileHeight;
					tile.hasPerspective = layer.perspective != null;
					...
					// copy the tile from the tileset onto our bitmap
					var bitmapData:BitmapData = new BitmapData(  currentTileset.tileWidth,  currentTileset.tileHeight, true, 0);
					bitmapData.copyPixels(currentTileset.bitmapData, new Rectangle(sourceX * currentTileset.tileWidth, sourceY * currentTileset.tileHeight, currentTileset.tileWidth, currentTileset.tileHeight), new Point(0, 0), null, null, true);
					tile.addChild(new Bitmap(bitmapData));
